...
if($_GET['act']=='attend'){
	...
	$t=C::t('#aljbd#aljbd_goods')->fetch_all_by_uid_bid_view($_G['uid'],$_GET['bid'],0,6);
	...
	$currpage=$_GET['page']?$_GET['page']:1;
	$perpage=$config['page'];
	$num=C::t('#aljbd#aljbd')->count_by_status(1,'',$_GET['type'],$_GET['subtype'],$_GET['region'],$_GET['subregion']);
	$allpage=ceil($num/$perpage);
	$start=($currpage-1)*$perpage;
	$recommendlist=C::t('#aljbd#aljbd')->fetch_all_by_recommend(1,0,10);;
	$bdlist=C::t('#aljbd#aljbd')->fetch_all_by_status(1,$start,$perpage,'',$_GET['type'],$_GET['subtype'],$_GET['region'],$_GET['subregion'],$_GET['order']);
	foreach($bdlist as $k=>$v){
		$bdlist[$k]['c']=C::t('#aljbd#aljbd_comment')->fetch_by_bid($v['id']);
	}
	$paging = helper_page :: multi($num, $perpage, $currpage, 'plugin.php?id=aljbd', 0, 11, false, false);
